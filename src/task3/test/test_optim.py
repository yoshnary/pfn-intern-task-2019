...
class TestModel(unittest.TestCase):
    ...
    def test_momentum_sgd(self):
        ...
        def test_sample(filename):
            with open(filename, 'r') as o:
                lr, momentum = list(map(float, o.readline().split()))
            ...
            model = TestModel(W, b)
            optimizer = optim.MomentumSGD(model, lr, momentum)
            for i in range(n):
            ...
            self.assertTrue(np.isclose(optimizer.mom, mom_ans).all(), filename)

        print('momentum_sgd')
        self.all_samples(os.path.join('test', 'momentum_sgd'), test_sample)
